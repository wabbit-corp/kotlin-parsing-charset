...
class CharSetSpec {
    ...
    private val genString =
        Gen.int(0..<20).flatMap { Gen.listOf(it, genChar) }.map { it.joinToString("") }

    private val genCharSet = genString.map { CharSet.of(it) }
    private val genCharSetPair = genCharSet.zip(genCharSet)
    private val genCharSetTriple =
        genCharSet.zip(genCharSet).zip(genCharSet).map {
    ...
    @Test
    fun `indiscernibility of identicals`() {
    ...
    @Test
    fun `identity of indiscernibles`() {
        genCharSetPair.foreach(100000) { (a, b) ->
            val areIndiscernible = properties.all { property -> property(a) == property(b) }
            if (areIndiscernible) {
                assertEquals(a, b)
            }
        }
    ...
    @Test
    fun `CharSet constructor roundtrip`() {
    ...
    @Test
    fun `contains is consistent with toSet contains`() =
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `containsAll is consistent with toSet containsAll`() =
    ...
    @Test
    fun `overlap is consistent with default implementation`() {
        genCharSetPair.foreachMin(SplittableRandom(), 100) { (a, b) ->
            ...
            assertEquals(
                default,
                overlap,
                "Overlap should be consistent with default implementation; $a, $b",
            )
        }
    }
    ...
    @Test fun `size returns 0 for CharSet none`() = assertTrue(CharSet.none.size == 0)

    @Test
    fun `size is consistent with toSet size`() =
        genCharSet.foreach(100) { charSet -> assertEquals(charSet.toSet().size, charSet.size) }

    @Test
    fun `isEmpty is consistent with size`() =
        genCharSet.foreach(100) { charSet -> assertEquals(charSet.size == 0, charSet.isEmpty()) }

    @Test
    fun `isNotEmpty is consistent with isEmpty`() =
        genCharSet.foreach(100) { charSet ->
            assertEquals(!charSet.isEmpty(), charSet.isNotEmpty())
        }

    @Test fun `isAll returns true for all`() = assertTrue(CharSet.all.isAll())

    @Test
    fun `isAll returns false for smaller sets`() =
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `toSet is consistent with toList`() =
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `toCharArray is consistent with toList`() =
        genCharSet.foreach(100) { charSet ->
    ...
    fun `iterator completeness`() {
        genCharSet.foreach(100) { charSet ->
            ...
            assertEquals(
                charSetAsList.size,
                iteratedChars.size,
                "Iterator should traverse all characters",
            )
            assertTrue(
                charSetAsList.containsAll(iteratedChars),
                "Iterator should traverse each character exactly once",
            )
            assertTrue(
                iteratedChars.containsAll(charSetAsList),
                "Iterator should traverse each character exactly once",
            )
        }
    }
        ...
        genCharSet.foreach(100) { charSet ->
            ...
            while (iterator1.hasNext() && iterator2.hasNext()) {
                ...
                assertEquals(
                    charFromIterator1,
                    charFromIterator2,
                    "Iterators should traverse CharSet in the same order",
                )
            }
    ...
    @Test
    fun `forEach is equivalent to toList`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `get is consistent with toList get`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `mapping with identity preserves CharSet`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `mapping with constant function yields CharSet of one element`() {
        genCharSet.foreach(100) { charSet ->
            if (charSet.isEmpty()) {
    ...
    @Test
    fun `mapping with composed function is equivalent to mapping twice`() {
        genCharSet.foreach(100) { charSet ->
            val mappedCharSet1 =
                charSet.map { (it.code / 2).toChar() }.map { (it.code / 3).toChar() }
            val mappedCharSet2 =
                charSet.map {
            ...
            assertEquals(
                mappedCharSet1,
                mappedCharSet2,
                "Mapping with composed function should be equivalent to mapping twice",
            )
        }
    }
    ...
    @Test
    fun `filter with {true} returns same set`() {
        genCharSet.foreach(100) { charSet ->
            val filteredCharSet = charSet.filter { true }
            assertEquals(
                charSet,
                filteredCharSet,
                "Filtering with a predicate that all elements satisfy should not change the CharSet",
            )
        }
    }

    @Test
    fun `filter with {false} returns empty set`() {
        genCharSet.foreach(100) { charSet ->
            val filteredCharSet = charSet.filter { false }
            assertEquals(
                CharSet.none,
                filteredCharSet,
                "Filtering with a predicate that no elements satisfy should result in an empty CharSet",
            )
        }
    }

    @Test
    fun `filter with {it in charSet} returns same set`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `filter with {it !in charSet} returns empty set`() {
        genCharSet.foreach(100) { charSet ->
            val filteredCharSet = charSet.filter { it !in charSet }
            assertEquals(
                CharSet.none,
    ...
    @Test
    fun `filter is consistent with toList filter`() {
        genCharSetPair.foreach(100) { (charSet1, charSet2) ->
            ...
            assertEquals(
                filteredList,
                filteredCharSet,
                "Filtering should be consistent with toList filter",
            )
        }
    }

    @Test
    fun `filter is consistent with intersect filter`() =
        genCharSetPair.foreach { (a, b) -> assertEquals(a.intersect(b), a.filter { it in b }) }

    @Test
    fun `filter composition`() {
    ...
    @Test
    fun `filter and union distributivity #1`() {
    ...
    @Test
    fun `filter and intersect distributivity`() {
    ...
    @Test
    fun `count with {true} returns size`() {
        genCharSet.foreach(100) { charSet ->
            val count = charSet.count { true }
            assertEquals(
                charSet.size,
                count,
                "Counting with a predicate that all elements satisfy should return the size of the CharSet",
            )
        }
    }

    @Test
    fun `count with {false} returns 0`() {
        genCharSet.foreach(100) { charSet ->
            val count = charSet.count { false }
            assertEquals(
                0,
                count,
                "Counting with a predicate that no elements satisfy should return 0",
            )
        }
    }

    @Test
    fun `count with {it in charSet} returns size`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `count with {it !in charSet} returns 0`() {
        genCharSet.foreach(100) { charSet ->
            val count = charSet.count { it !in charSet }
            assertEquals(
                0,
    ...
    @Test
    fun `count is equivalent to filter composed with size`() {
        genCharSet.foreach(100) { charSet ->
            val count = charSet.count { it in charSet }
            val filteredCharSet = charSet.filter { it in charSet }
            assertEquals(
                filteredCharSet.size,
                count,
                "Counting should be equivalent to filtering and then taking the size",
            )
        }
    }

    @Test
    fun `count is consistent with toList count`() {
        genCharSetPair.foreach(100) { (charSet1, charSet2) ->
            ...
            assertEquals(
                toListCountResult,
                countResult,
                "Counting should be consistent with toList count",
            )
        }
    }
    ...
    @Test
    fun `union with none, all, and itself`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `union is consistent with toSet union`() {
    ...
    @Test
    fun `consistency of containsAll with union`() {
        genCharSetTriple.foreach(100) { (a, b, c) ->
    ...
    @Test
    fun `intersection with none, all, and itself`() {
        genCharSet.foreach(100) { charSet ->
    ...
    @Test
    fun `intersection is commutative`() {
        genCharSetPair.foreach(100) { (charSet1, charSet2) ->
    ...
    @Test
    fun `intersection is associative`() {
        genCharSetTriple.foreach(100) { (charSetA, charSetB, charSetC) ->
    ...
    @Test
    fun `intersection is consistent with toSet intersect`() {
        genCharSetPair.foreach(100) { (charSetA, charSetB) ->
            ...
            assertEquals(
                setIntersection,
                intersection,
                "Intersection should be consistent with toSet intersect",
            )
        }
    }

    @Test
    fun `consistency of containsAll with intersection`() {
        genCharSetTriple.foreach(100) { (a, b, c) ->
            // If a contains all elements of b and c,
    ...
    @Test
    fun `difference has default implementation in terms of inverse and intersect`() =
        genCharSetPair.foreach { (charSet1, charSet2) ->
            assertEquals(charSet1.intersect(charSet2.invert()), charSet1.difference(charSet2))
        }

    @Test
    fun `minus has default implementation in terms of difference`() =
        genCharSetPair.foreach { (charSet1, charSet2) ->
            assertEquals(charSet1.difference(charSet2), charSet1 - charSet2)
        }

    @Test
    fun `subtracting CharSet from itself yields empty set`() =
        genCharSet.foreach { assertEquals(CharSet.none, it - it) }

    @Test
    fun `subtracting empty set from any CharSet yields original CharSet`() =
        genCharSet.foreach { assertEquals(it, it - CharSet.none) }

    @Test
    fun `subtracting CharSet all from any CharSet yields empty set`() =
        genCharSet.foreach { assertEquals(CharSet.none, it - CharSet.all) }

    @Test
    fun `subtracting one CharSet from another is equivalent to intersecting with the inverse`() {
        genCharSetPair.foreach { (a, b) -> assertEquals(a intersect b.invert(), a - b) }
    }

    // Union/Intersection/Inverse properties
    @Test
    fun `distributive property of union over intersection`() {
        genCharSetTriple.foreach(100) { (a, b, c) ->
            ...
            assertEquals(
                unionThenIntersect,
                intersectThenUnionA,
                "Distributive property of union over intersection failed",
            )
        }
    }

    @Test
    fun `distributive property of intersection over union`() {
        genCharSetTriple.foreach(100) { (a, b, c) ->
            ...
            assertEquals(
                intersectThenUnion,
                unionThenIntersectA,
                "Distributive property of intersection over union failed",
            )
        }
    }

    @Test
    fun `union and intersection on subsets`() {
        genCharSetPair.foreach(100) { (a, b) ->
            ...
            // Test the intersection property: if a is a subset of b, then a intersect b should be
            // equal to a
    ...
    @Test
    fun `other properties of union and intersection`() {
        genCharSetPair.foreach { (a, b) ->
            ...
            assertEquals(a.size + b.size - (a intersect b).size, (a union b).size)
    ...
    @Test
    fun `isSubsetOf is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) -> assertEquals((a intersect b) == a, a isSubsetOf b) }

    @Test
    fun `isSupersetOf is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) -> assertEquals((a intersect b) == b, a isSupersetOf b) }

    @Test
    fun `isDisjointWith is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) ->
            assertEquals((a intersect b).isEmpty(), a isDisjointWith b)
        }

    @Test
    fun `isOverlappingWith is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) ->
            assertEquals((a intersect b).isNotEmpty(), a isOverlappingWith b)
        }

    @Test
    fun `isProperSubsetOf is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) ->
            assertEquals((a intersect b) == a && a != b, a isProperSubsetOf b)
        }

    @Test
    fun `isProperSupersetOf is consistent with default implementation`() =
        genCharSetPair.foreach { (a, b) ->
    ...
    // toString properties
    @Test
    fun `equality on sets is equivalent to equality on toString`() =
        genCharSetPair.foreach { (a, b) -> assertEquals(a == b, a.toString() == b.toString()) }

    // Constructors
    @Test
    fun `constructor tests`() {
